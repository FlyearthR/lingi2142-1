...
do
	for p in "udp" "tcp";
	do
		ip6tables -A FORWARD	-s $i	-p $p	-m multiport --dports 53,80,443,5001	-j ACCEPT
		ip6tables -A FORWARD	-s $i	-p $p	-m multiport --dports 53,80,443,5001	-j ACCEPT
	done;
done;
...
# Protection against flooding and DoS/DdoS
ip6tables -A INPUT	-p icmpv6 --icmpv6-type 128/0	-m limit --limit 10/minute	-j ACCEPT # Echo request
ip6tables -A FORWARD	-p icmpv6 --icmpv6-type 128/0	-m limit --limit 10/minute	-j ACCEPT # Echo request
ip6tables -A INPUT	-p icmpv6 --icmpv6-type 135/0	-m limit --limit 10/minute	-j ACCEPT # Neighbor solicitation
ip6tables -A FORWARD	-p icmpv6 --icmpv6-type 135/0	-m limit --limit 10/minute	-j ACCEPT # Neighbor solicitation
ip6tables -A INPUT	-p icmpv6 --icmpv6-type 128/0	-j DROP
ip6tables -A FORWARD	-p icmpv6 --icmpv6-type 128/0	-j DROP
ip6tables -A INPUT	-p icmpv6 --icmpv6-type 135/0	-j DROP
ip6tables -A FORWARD	-p icmpv6 --icmpv6-type 135/0	-j DROP
...
# Record some dropped packets in files
ip6tables -N LOGS
